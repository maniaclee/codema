 ...
 */
public class MybatisMeyouTest extends BaseTest {
    @Test
    public void mybatis() throws Exception {
        MavenConfig maven =  MavenConfig.parse(path(myWorkspace(),"meyouservice"));
        ...
        for (Machine<SqlTable, SqlTable> sql : SqlMachineFactory.fromJdbcUrl("jdbc:mysql://103.37.159.247:3306/lvbby?characterEncoding=UTF-8",
            "lee", "#Caonima123", "meyou.*")) {



            SqlTable table = sql.getSource();
            table.setName(table.getName().replace("Meyou",""));
            table.setNameInDb(table.getNameInDb().replace("meyou_",""));



            /** entity */
            Machine<JavaClass, JavaClass> bean = new JavaBeanMachine()
                    .javaClassNameParser(JavaClassNameParserFactory.format("com.meyou.entity.%sEntity"))
                    .destRootDir(maven.getDestSrcRoot());

            /** DTO */
            Machine<JavaClass, JavaClass> dto = new JavaBeanMachine()
                    .javaClassNameParser(JavaClassNameParserFactory.format("com.meyou.dto.%sDTO"))
                    .destRootDir(maven.getDestSrcRoot());
            ...
            mybatisXml.setMapperName(sqlTable -> String.format("com.meyou.dao.%sDao", sqlTable.getName()));
            ...
            mapper.setDestClassName(source -> String.format("com.meyou.dao.%sDao", source.getName().replace("Entity","")));
...
//            /** repository */
            ...
            repo.javaClassNameParser(JavaClassNameParserFactory.format("com.meyou.repo.%sRepo"));
